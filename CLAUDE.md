...
| `Contrato` | 4 docs GED fixos (`comprovante_renda`/`contrato_social`/`recibo_chaves`/`comprovante_anual`, 1 arquivo cada) anexados no **detail** via `contrato_anexar_documento`, não no form. Anexos **múltiplos** ficam no model filho `DocumentoContrato` (`related_name='documentos_pessoais'`, 1 linha/arquivo) via `contrato_documento_pessoal_upload`/`_delete` — sem formset, itera `request.FILES.getlist`. **O locador exibido no PDF gerado não é `imovel.proprietario`** — é sempre `settings.SHELTER_LOCADOR` (dados fixos da administradora); o `Proprietario` cadastrado continua normal no resto do sistema. `finalidade`/`local_assinatura`/`data_assinatura` alimentam o PDF jurídico. |
